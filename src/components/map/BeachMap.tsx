...
export default function AdminDashboard() {
  ...
  const renderAdminSpot = (num: number) => {
    ...
    // CROMIE VARIATE SU BASE ARANCIONE
    let btnClass = "bg-slate-900 text-slate-300 border-slate-800 hover:bg-slate-800 hover:text-white";
    if (isManuallyBlocked) {
      btnClass = "bg-amber-600/20 border-amber-500/60 text-amber-500 hover:bg-amber-600/30 font-bold";
    } else if (isDailyLocalBlock) {
      btnClass = "bg-orange-500/30 border-orange-500/80 text-orange-400 hover:bg-orange-500/40 font-bold";
    } else if (isBooked) {
      btnClass = "bg-red-500/20 border-red-500/60 text-red-400 hover:bg-red-500/30";
    } else if (correspondingSpot) {
      // Libero / Attivo -> Arancione soft di base
      btnClass = "bg-orange-500/10 border-orange-500/30 text-orange-300 hover:bg-orange-500/20";
    }
    ...
    return (
      <button
        ...
        className={`w-9 h-11 border text-[11px] rounded-lg flex items-center justify-center font-black transition relative ${btnClass} ${selectedSpot?.internal_code === num.toString() ? 'ring-2 ring-orange-500 border-transparent shadow-lg shadow-orange-500/20 scale-105' : ''}`}
      >
        {num}
        {isBooked && (
          <span className={`absolute top-1 right-1 w-1.5 h-1.5 rounded-full committees ${isDailyLocalBlock ? 'bg-orange-400 animate-pulse' : 'bg-red-500'}`} />
        )}
      </button>
    );
  };
          ...
          <div className="xl:col-span-3 bg-slate-900 border border-slate-800 p-5 rounded-3xl space-y-4 shadow-xl overflow-x-auto">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-800 pb-4 gap-3 min-w-[920px]">
              ...
              <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-wider">
                <span className="flex items-center gap-1.5 text-orange-300"><span className="w-2.5 h-2.5 rounded-md bg-orange-500/10 border border-orange-500/30"></span> Attivo</span>
                ...
                <div className="flex items-center justify-between bg-slate-950 p-3 rounded-2xl border border-slate-800">
                  ...
                  <span className={`px-2 py-0.5 text-[9px] rounded-lg font-black uppercase border ${
                    selectedSpot._booking?.booking_category === 'Giornaliero in loco'
                      ...
                      : selectedSpot.is_available ? 'bg-orange-500/10 text-orange-400 border-orange-500/30' : 'bg-amber-500/10 text-amber-400 border-amber-500/30'
                  }`}>
                    {selectedSpot._booking?.booking_category === 'Giornaliero in loco' 
                      ? 'In Loco' 
