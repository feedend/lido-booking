...
export default function AdminBookings() {
  ...
  // Funzione di caricamento dati da Supabase
  ...
  // Protezione Rotta Admin ed esecuzione Fetch
  useEffect(() => {
    let isMounted = true;
  ...
  // Logica di Filtraggio e Ricerca combinata (Include il parsing testuale)
  useEffect(() => {
    let result = [...bookings];
    
    // Generazione stringhe data per i filtri veloci
    ...
    // 3. Filtro di Ricerca Testuale Avanzato
    if (searchTerm.trim() !== '') {
      ...
      result = result.filter(b => {
        ...
        // Eseguiamo un pre-parsing anche in ricerca per trovare l'email se digitata
        let parsedName = '';
        let parsedEmail = '';
  ...
  // Modifica dello stato della prenotazione (Approva / Rifiuta)
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans antialiased">
      ...
      <main className="max-w-[1600px] mx-auto p-4 md:p-6 space-y-6">
        ...
        {/* Pannello dei Filtri e della Barra di Ricerca */}
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-lg">
              ...
              <tbody className="divide-y divide-slate-800/60 text-sm">
                {loading ? (
                  ...
                  filteredBookings.map((booking) => {
                    // Conversione e pulizia prezzo dal DB (gestione dei NULL)
                    const parsedPrice = booking.total_price ? Number(booking.total_price) : 0;

                    // Valori di default per l'anagrafica
                    let clientName = "Ospite Diretto";
                    let clientEmail = "Nessuna Mail";

                    // LOGICA DI PARSING AVANZATA (Risoluzione del tracciamento da screenshot 3.png)
                    if (booking.booking_category && booking.booking_category.includes('|')) {
                      const parts = booking.booking_category.split('|').map(p => p.trim());
                      
                      // Dalla struttura "45 | Nome/Gruppo | email@dominio.com"
                      if (parts[1]) clientName = parts[1];
                      if (parts[2]) clientEmail = parts[2];
                    } else {
                      // Fallback relazionale standard su tabella profiles
                      if (booking.profiles?.full_name) {
                        clientName = booking.profiles.full_name;
                      } else if (booking.booking_category) {
                        clientName = booking.booking_category;
                      }
          ...
          {/* Footer del Riepilogo Finanziario */}
          {!loading && filteredBookings.length > 0 && (
            <div className="bg-slate-950/20 border-t border-slate-800 px-6 py-3 text-xs text-slate-400 flex justify-between items-center">
              <span>Mostrate {filteredBookings.length} prenotazioni complessive</span>
