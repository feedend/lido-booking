...
// Interfaccia estesa per includere i dati in JOIN da spots e profiles
...
export default function AdminBookings() {
  ...
  const fetchBookings = useCallback(async () => {
    ...
    try {
      // Query relazionale: recupera la prenotazione + internal_code dello spot + info del profilo utente
  ...
  // Controllo sessione utente ed esecuzione fetch
  useEffect(() => {
    let isMounted = true;
  ...
  // Gestione dei filtri (Ricerca, Data e Stato)
  useEffect(() => {
    let result = [...bookings];
    ...
    // 3. Filtro di Ricerca Testuale (Cerca per Ombrellone, Nome Cliente o Email)
    if (searchTerm.trim() !== '') {
      ...
      result = result.filter(b => {
        ...
        const category = b.booking_category?.toLowerCase() || '';
  ...
  // Funzione per aggiornare al volo lo stato di una prenotazione (es. da pending a confirmed o cancelled)
  ...
  const getSlotLabel = (slot: string) => {
    if (slot === 'morning') return 'Mattina (AM)';
    if (slot === 'afternoon') return 'Pomeriggio (PM)';
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans antialiased">
      ...
      <nav className="bg-slate-900 border-b border-slate-800 px-6 py-4 flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Pannello di controllo flussi e pagamenti</p>
          </div>
        </div>
        <button 
          onClick={fetchBookings} 
          className="p-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 transition flex items-center gap-2 text-xs font-medium"
        >
      ...
      <main className="max-w-[1600px] mx-auto p-4 md:p-6 space-y-6">
        ...
        {/* Barra dei filtri e strumenti */}
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex flex-col md:flex-row gap-4 justify-between items-stretch md:items-center shadow-md">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input 
              type="text" 
              placeholder="Cerca per ombrellone, nome cliente, email..."
              value={searchTerm}
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex bg-slate-950 p-1 border border-slate-800 rounded-lg text-xs font-medium">
              <button 
                ...
                className={`px-3 py-1.5 rounded-md transition ${dateFilter === 'today' ? 'bg-blue-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}
              >
                Oggi
              </button>
              <button 
                onClick={() => setDateFilter('tomorrow')} 
                className={`px-3 py-1.5 rounded-md transition ${dateFilter === 'tomorrow' ? 'bg-blue-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}
              >
                Domani
              </button>
              <button 
                onClick={() => setDateFilter('all')} 
                className={`px-3 py-1.5 rounded-md transition ${dateFilter === 'all' ? 'bg-blue-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}
              >
                Tutte
              </button>
            </div>
            ...
            <select 
              ...
              className="bg-slate-950 border border-slate-800 px-3 py-2 rounded-lg text-xs text-slate-300 focus:outline-none focus:border-blue-500 cursor-pointer"
            >
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-lg">
                ...
                {loading ? (
                  <tr>
                    <td colSpan={8} className="text-center py-12 text-slate-400">
                      <RefreshCw className="animate-spin inline-block mr-2 h-4 w-4 text-blue-500" />
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="font-medium text-slate-200">
                            {booking.profiles?.full_name || 'Utente Anonimo'}
                          </span>
                          <span className="text-xs text-slate-400 font-mono">
                            {booking.profiles?.email || 'Nessuna Email'}
                          </span>
                        </div>
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-1.5">
                          {booking.status !== 'confirmed' && (
                            <button
                              ...
                              className="px-2 py-1 bg-emerald-600/10 text-emerald-400 hover:bg-emerald-600 hover:text-white border border-emerald-500/20 text-xs font-semibold rounded transition"
                          ...
                          {booking.status !== 'cancelled' && (
                            <button
                              ...
                              className="px-2 py-1 bg-red-600/10 text-red-400 hover:bg-red-600 hover:text-white border border-red-500/20 text-xs font-semibold rounded transition"
          ...
          {/* Footer riepilogativo della tabella */}
          {!loading && filteredBookings.length > 0 && (
            <div className="bg-slate-950/20 border-t border-slate-800 px-6 py-3 text-xs text-slate-400 flex justify-between items-center">
              <span>Mostrate {filteredBookings.length} prenotazioni valide</span>
